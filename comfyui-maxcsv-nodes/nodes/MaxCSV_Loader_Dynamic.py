...
class Max_CSV_Loader_Dynamic:
    ...
    def get_output_info(self, csv_file):
        global csv_path
        ...
        return_types = tuple(["STRING"] * len(headers))
        return_names = tuple(headers)
    ...
    def browse_csv(self, csv_file, selection_type="single", selected_row="", filter_text="", iteration_index=0, extra_pnginfo=None):
        if self.selected_csv != csv_file:
            self.RETURN_TYPES, self.RETURN_NAMES = self.get_output_info(csv_file)
            self.selected_csv = csv_file

        # The rest of the browse_csv function remains the same
        # ...
